...
class NcaafBetHistoryCard extends StatelessWidget {
  const NcaafBetHistoryCard(
      {Key key, @required this.betHistoryData, this.isParlayLeg = false})
      : assert(betHistoryData != null),
        super(key: key);

  final NcaafBetData betHistoryData;
              ...
              RichText(
                text: TextSpan(
                  text: isMoneyline || isPointSpread
                      ? '${betHistoryData.betTeam == 'home' ? '${betHistoryData.homeTeamSchool ?? ''} ${betHistoryData.homeTeamName}' : '${betHistoryData.awayTeamSchool ?? ''} ${betHistoryData.awayTeamName}'}'
                      : '',
